...
A configuration file can be found in the /config directory. There are a few configuration details that you will need to specify before running Jently. The comments in the file should hopefully make it easy to get everything set up right. Store configuration that you don't want checked in to the repo in your +.env+ file, which can be derived by copying the +.env.example+ file to +.env+ and editing the settings to suit your needs.
